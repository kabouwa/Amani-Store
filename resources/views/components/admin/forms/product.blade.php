...
<form action={{ $product ? route('admin.products.update', $product->slug) : route('admin.products.store') }} method="POST" novalidate enctype="multipart/form-data"
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <span id="imagesCount" class="text-xs text-gray-400">{{ count($product->images) ?? 0 }} / 10</span>
        </div>
        {{-- Drop zone --}}
        <div id="dropZone"
            ...
            <div class="flex flex-col items-center gap-3 pointer-events-none">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">PNG, JPG, JPEG — jusqu'à {{ $product ? 10 - count($product->images) : 10 }} images</p>
                </div>
            </div>
        </div>
        @error('images')
            ...
            {{-- Preview grid --}}
            <div id="imagesPreview" class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3 mt-5" data-img-count="{{ count($product->images) }}"></div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            ...
            <input type="text" name="title" id="title" value="{{ old('title', $product ? $product->title : '') }}" required
        ...
        <div class="relative">
            ...
            <select name="category_id" id="category_id" required
                    class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-gray-900 bg-white cursor-pointer
                           focus:outline-none focus:ring-2 focus:ring-amani focus:border-amani transition">
                <option value="" {{ old('category_id', $product?->category?->id ) === null  ? 'selected' : '' }}>Non Classé</option>
        ...
        <div>
            ...
            <input type="number" min="0" name="stock" id="stock" value={{ old('stock', $product ? $product->stock : 0) }} required
                   class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-gray-900
                          focus:outline-none focus:ring-2 focus:ring-amani focus:border-amani transition">
            @error('stock')
                <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
    ...
    <div class="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
        ...
        @unless (is_null($product))
        <button type="button" class="js-delete-btn cursor-pointer text-amani hover:bg-gray-100 px-6 py-2.5 rounded-lg transition flex items-center gap-2"
