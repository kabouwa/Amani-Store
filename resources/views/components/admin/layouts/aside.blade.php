...
<aside id="sidebar"
    ...
    <nav class="p-4 space-y-1">
        ...
        <a href="{{ route('admin.dashboard') }}"
        class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-gray-700 hover:bg-amani/10 hover:text-amani transition
                {{ request()->routeIs('admin.orders.*') ? 'bg-amani/10 text-amani font-semibold' : '' }}">
            <i class="fa-solid fa-box w-4 text-center"></i> Commandes
        </a>

        <a href="{{ route('admin.products.index') }}"
        class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-gray-700 hover:bg-amani/10 hover:text-amani transition
                {{ request()->routeIs('admin.products.*') ? 'bg-amani/10 text-amani font-semibold' : '' }}">
            <i class="fa-solid fa-bag-shopping w-4 text-center"></i> Produits
        </a>

        <a href="{{ route('admin.categories.index') }}"
            class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-gray-700 hover:bg-amani/10 hover:text-amani transition
                    {{ request()->routeIs('admin.categories.*') ? 'bg-amani/10 text-amani font-semibold' : '' }}">
                <i class="fa-solid fa-tags w-4 text-center"></i> Catégories
            </a>

        <a href="{{ route('admin.dashboard') }}"
        class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-gray-700 hover:bg-amani/10 hover:text-amani transition
                {{ request()->routeIs('admin.customers.*') ? 'bg-amani/10 text-amani font-semibold' : '' }}">
